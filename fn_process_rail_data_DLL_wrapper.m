...
no_modes = numel(rail_tester_in.disperse.mode);
no_freq_pts = numel(rail_tester_in.disperse.mode(1).freq);
no_ms_pts = size(rail_tester_in.disperse.mode(1).ms_x, 2);
for m = 1:no_modes
    if m == 1
    ...
    disperse_freq(:,1,m) = rail_tester_in.disperse.mode(m).freq;
    disperse_vph(:,1,m) = rail_tester_in.disperse.mode(m).vph;
    disperse_vgr(:,1,m) = rail_tester_in.disperse.mode(m).vgr;
    disperse_waveno(:,1,m) = rail_tester_in.disperse.mode(m).waveno;
    disperse_ms_x(:,:,m) = rail_tester_in.disperse.mode(m).ms_x;
    disperse_ms_y(:,:,m) = rail_tester_in.disperse.mode(m).ms_y;
    disperse_ms_z(:,:,m) = rail_tester_in.disperse.mode(m).ms_z;
end
