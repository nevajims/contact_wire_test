function [rail_tester, success] = convert_rla_file(rla_fname, fe_fname, hardware_fname);
success = 0;
rail_tester = 0;
...
%load rla file
tic;
[rail_tester.raw_data, rail_tester.input_signal, rla_success] = load_rla_file(rla_fname);
if ~rla_success
...
for ii = 1:size(rail_tester.raw_data.time_data, 2);
	sgn = trans_pos_phasings(rail_tester.raw_data.tx_pos(ii)) * trans_pos_phasings(rail_tester.raw_data.rx_pos(ii));
	rail_tester.raw_data.time_data(:, ii) = rail_tester.raw_data.time_data(:, ii) * sgn;
end;
