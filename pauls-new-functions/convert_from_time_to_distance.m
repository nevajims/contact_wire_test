function proc_data_out = convert_from_time_to_distance(out_time_data, proc_data, disperse, ms_matrix_modes_to_use, freq_filter, suppress_display);
tic;
...
for ii=1:length(ms_matrix_modes_to_use);
    %linear interpolation now used to allow for data which is not
    %continuous, added by mark jan06
    kk = find(disperse.mode(ms_matrix_modes_to_use(ii)).freq>=freq_filter,1);
	vgr(ms_matrix_modes_to_use(ii)) = interp1(...
		disperse.mode(ms_matrix_modes_to_use(ii)).freq(kk-1:kk), ...
		disperse.mode(ms_matrix_modes_to_use(ii)).vgr(kk-1:kk), ...
		freq_filter, 'nearest');
	k(ms_matrix_modes_to_use(ii)) = interp1(...
		disperse.mode(ms_matrix_modes_to_use(ii)).freq(kk-1:kk), ...
		disperse.mode(ms_matrix_modes_to_use(ii)).waveno(kk-1:kk), ...
		freq_filter, 'nearest');
end;
