...
data_fname = 'c:\tmp\Export-1883-16kHz.mat'

%fe_fname = 'D:\grail\matlab\fe-data\2D-models\CEN60_course.mat';
...
options.raw_data_zero_dead_time = 5.0e-4;
%default options - mode shape matrix calculation
options.ms_matrix_modes_to_use = [3,4,5,8,9];%modes used in calculation of matrices
...
options.ms_matrix_calc_freqs = linspace(13.5e3,18.5e3,11); %frequencies to calc mode shape matrix at for linear interpolation
options.ms_matrix_ignore_pulse_echo = 1;
options.ms_matrix_bidi_calc = 1; %turn on to include transmit in +ve direction and receive in +ve direction combos in calc
...
%plot options
plot_options.db_range = 40;
%plot_options.tx_modes = [8,8,8,8,8,10,10,10,10,10];
%plot_options.rx_modes = [3, 5, 7, 8, 10, 3, 5, 7, 8, 10];
% 
plot_options.tx_modes = [8, 8, 8, 8, 8];
plot_options.tx_modes = [3,5,8,9];
plot_options.rx_modes = [3,5,8,9];

plot_options.max_range = 15;
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%load the file
